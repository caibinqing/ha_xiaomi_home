...
from .specs.specv2entity import (
    ...
    SPEC_PROP_TRANS_MAP,
    SPEC_SERVICE_TRANS_MAP, PROPRIETARY_SERVICE_EXCLUDE
)
...
class MIoTDevice:
    ...
    def gen_service_entity_id_unofficial(
        self, ha_domain: str, siid: int, description: str
    ) -> list[str]:
        """Return the service entity ids generated by an unofficial patch.

        The patch slugified the description, and an earlier revision of it
        left a trailing separator behind when the description was empty once
        slugified.
        """
        entity_id = (
            f'{ha_domain}.{self._model_strs[0][:9]}_{self.did_tag}_'
        ...
        return [
            f'{entity_id}_{slugify_name(description)}',
            f'{entity_id}_']
        ...
        for service in self.spec_instance.services:
            ...
            for prop in service.properties:
                ...
                if prop.platform in ['number', 'select', 'switch', 'text'] and (
                    (
                        prop.proprietary
                        and prop.service.name not in PROPRIETARY_SERVICE_EXCLUDE
                    )
